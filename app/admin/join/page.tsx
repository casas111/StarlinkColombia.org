...
export default async function JoinAdmin({searchParams}:{searchParams:Promise<{token?:string}>}){
  ...
  if(!invite)return <main className="auth-shell"><section className="auth-card"><p className="eyebrow">Enlace inválido</p><h1>Esta invitación ya no está activa.</h1><p>Pide a un administrador que genere un enlace nuevo.</p><a className="back-link" href="/">← Volver</a></section></main>;
  const user=await getChatGPTUser();if(!user)return <main className="auth-shell"><section className="auth-card"><a className="brand" href="/"><span className="signal-dot"/>Conecta Colombia</a><p className="eyebrow">Invitación de administrador</p><h1>Únete al equipo.</h1><p>Ingresa con ChatGPT para activar tu acceso al portal de administración.</p><a className="primary-button inline-button" href={chatGPTSignInPath(`/admin/join?token=${token}`)}>Aceptar invitación</a></section></main>;
